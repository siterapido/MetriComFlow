...
export default (globalThis as any).Deno?.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: subscription, error: subscriptionError } = await adminClient
      ...
      .select("id, organization_id, plan_id, stripe_subscription_id, metadata, cancel_at_period_end, status")
    ...
    if (!membership || membership.role !== "owner") {
      return new Response(
        JSON.stringify({ error: "Apenas proprietários podem reativar a assinatura." }),
    ...
    if (stripeSubscriptionId) {
      try {
        ...
        // For subscriptions already canceled at Stripe, surface clearer error
        const message =
          err instanceof Error && "code" in err
            ? `Stripe: ${(err as any).code ?? err.message}`
            : err instanceof Error
              ? err.message
              : String(err);
        throw new Error(`Não foi possível reativar assinatura na Stripe: ${message}`);
      }
    }
    ...
    console.error("reactivate-stripe-subscription error:", err);
