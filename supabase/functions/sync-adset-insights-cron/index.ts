...
Deno.serve(async (req) => {
  try {
    ...
    console.log('🔄 Iniciando sincronização automática de insights de Ad Sets...')
    
    // Registrar início da execução
    const { data: logEntry } = await supabase
      .from('cron_job_logs')
      .insert({
        job_name: 'sync_adset_insights_cron',
        status: 'running',
    ...
    for (const org of organizations) {
      try {
        ...
        console.log(`🔄 Sincronizando insights de Ad Sets da organização ${org.name} (${org.id})`)
        console.log(`📅 Intervalo: ${dateRange.since} até ${dateRange.until}`)
        
        // Chamar a função de sincronização de insights de Ad Sets
        const syncResponse = await supabase.functions.invoke('sync-adset-insights', {
          body: {
        ...
        if (syncResponse.error) {
          console.error(`❌ Erro ao sincronizar insights de Ad Sets para ${org.name}:`, syncResponse.error)
          results.push({
            organization_id: org.id,
          ...
          console.log(`✅ Insights de Ad Sets sincronizados com sucesso para ${org.name}`)
          results.push({
            organization_id: org.id,
            organization_name: org.name,
            success: true,
    ...
    console.log('✅ Sincronização automática de insights de Ad Sets concluída')
