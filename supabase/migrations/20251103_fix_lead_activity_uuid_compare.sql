-- Fix UUID comparisons in log_lead_activity to avoid casting '' to uuid
-- This migration replaces COALESCE(..., '') comparisons on UUID columns with
-- proper IS DISTINCT FROM checks (which handle NULLs safely).
...
begin
  ...
  if tg_op = 'INSERT' then
    ...
    -- Source/campaign change (UUID-safe for campaign_id)
    if coalesce(old.source, '') is distinct from coalesce(new.source, '')
