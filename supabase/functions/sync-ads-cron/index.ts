...
Deno.serve(async (req) => {
  try {
    ...
    console.log('🔄 Iniciando sincronização automática de Ads...')
    
    // Registrar início da execução
    const { data: logEntry } = await supabase
      .from('cron_job_logs')
      .insert({
        job_name: 'sync_ads_cron',
        status: 'running',
    ...
    for (const org of organizations) {
      try {
        ...
        console.log(`🔄 Sincronizando Ads da organização ${org.name} (${org.id})`)
        console.log(`📅 Intervalo: ${dateRange.since} até ${dateRange.until}`)
        
        // Chamar a função de sincronização de Ads
        const syncResponse = await supabase.functions.invoke('sync-ads', {
          body: {
        ...
        if (syncResponse.error) {
          console.error(`❌ Erro ao sincronizar Ads para ${org.name}:`, syncResponse.error)
          results.push({
            organization_id: org.id,
          ...
          console.log(`✅ Ads sincronizados com sucesso para ${org.name}`)
    ...
    console.log('✅ Sincronização automática de Ads concluída')
