...
Deno.serve(async (req) => {
  try {
    ...
    console.log('🔄 Iniciando sincronização automática de insights de Ads...')
    
    // Registrar início da execução
    const { data: logEntry } = await supabase
      .from('cron_job_logs')
      .insert({
        job_name: 'sync_ad_insights_cron',
        status: 'running',
    ...
    for (const org of organizations) {
      try {
        ...
        const historyDays = org.organization_sync_config?.[0]?.ad_history_days || 7
        
        // Calcular intervalo de datas baseado no histórico do plano (últimos 7 dias para insights)
        const endDate = new Date()
        const startDate = new Date()
        startDate.setDate(startDate.getDate() - Math.min(historyDays, 7))
        ...
        console.log(`🔄 Sincronizando insights de Ads da organização ${org.name} (${org.id})`)
        console.log(`📅 Intervalo: ${dateRange.since} até ${dateRange.until}`)
        
        // Chamar a função de sincronização de insights de Ads
        const syncResponse = await supabase.functions.invoke('sync-ad-insights', {
          body: {
        ...
        if (syncResponse.error) {
          console.error(`❌ Erro ao sincronizar insights de Ads para ${org.name}:`, syncResponse.error)
          results.push({
            organization_id: org.id,
          ...
          console.log(`✅ Insights de Ads sincronizados com sucesso para ${org.name}`)
          results.push({
            organization_id: org.id,
            organization_name: org.name,
            success: true,
    ...
    console.log('✅ Sincronização automática de insights de Ads concluída')
